...
const translations = {
    en: {
        ...
        host2_resume: `<p>Born on December 26, 1953.</p><ul><li>Graduated from a music boarding school (piano class), then the Lviv Conservatory in 1980.</li><li><b>Until 1986:</b> Continued his development at the Lviv Philharmonic with the ensemble "Vatra" alongside Oksana and Igor Bilozir.</li><li>Next, worked at the Rivne Philharmonic with the group "Zhayvir", which later moved to Moscow and successfully toured the USSR with the famous Russian singer Irina Ponarovskaya until 1990.</li><li><b>Until 2000:</b> Returned to Lviv for studio work with Igor Bilozir.</li><li>Later, worked as a Sound Engineer at Lviv State Television and Radio Company (Lviv Radio).</li><li><b>From 2010:</b> Producer and sound engineer for Oksana Lernatovych's internet radio project "Ukraine in the heart of everyone," broadcasted by the Ukrainian programs editorial office of Radio Pittsburgh, Pennsylvania, USA.</li></ul><p>To collaborate, contact: Lzm211253@gmail.com. Some programs can also be found on YouTube under the project's name.</p>`,
...
async function loadEpisodes() {
    ...
    // Try local cached feed first (no CORS)
    try {
        console.log("Attempting to load local episodes.xml...");
        const response = await fetch(localFeedUrl);
        if (response.ok) {
            xmlText = await response.text();
            console.log("Successfully loaded episodes from local cache.");
        } else {
...
function startSubtitleRotation() {
    ...
    const texts = ["Ukraine In The Heart of Everyone", "Україна у серці одна", "Christ Among Us", "Христос посеред нас"];
    let currentIndex = 0;
